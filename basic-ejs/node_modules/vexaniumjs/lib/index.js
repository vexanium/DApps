...
Object.assign(Vex, {
  ...
  Testnet: function Testnet(config) {
    console.error('deprecated, change Vex.Testnet(..) to just Vex(..)');
    return Vex(config);
  },

  /** @deprecated */
  Localnet: function Localnet(config) {
    console.error('deprecated, change Vex.Localnet(..) to just Vex(..)');
    return Vex(config);
  }
});

function createEos(config) {
  var network = config.httpEndpoint != null ? VexApi(config) : null;
  config.network = network;

  var abis = [];
  var abiCache = AbiCache(network, config);
  abis.push(abiCache.abi('vex.null', eosio_null));
  abis.push(abiCache.abi('vex.token', token));
  abis.push(abiCache.abi('vexcore', system));
  ...
  var eos = mergeWriteFunctions(config, VexApi, structs, abis);
  ...
  Object.assign(eos, {
    ...
    // Repeat of static Vex.modules, help apps that use dependency injection
    modules: {
      format: format
    }
  });
...
/**
  ...
  @arg {object} VexApi - api[VexApi] read-only api calls
  @return {object} - read and write method calls (create and sign transactions)
  @throw {TypeError} if a funciton name conflicts
*/
function mergeWriteFunctions(config, VexApi, structs, abis) {
  var network = config.network;


  var merge = Object.assign({}, network);

  var writeApi = writeApiGen(VexApi, network, structs, config, abis);
  throwOnDuplicate(merge, writeApi, 'Conflicting methods in VexApi and Transaction Api');
