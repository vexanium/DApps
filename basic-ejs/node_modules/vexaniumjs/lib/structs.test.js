      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _context.next = 3;
            return _regenerator2.default.awrap(eos.contract('vexcore'));
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            authority = eosio.fc.structs.authority;
            pubkeys = ['VEX7wBGPvBgRVa4wQN2zm5CjgBF6S7tP7R3JavtSa2unHUoVQGhey', 'VEX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV'];
...
describe('Eosio Abi', function () {
  ...
  function checkContract(name) {
    it(name + ' contract parses', function (done) {
      ...
      eos.contract('vex.token', function (error, vex_token) {
        assert(!error, error);
        assert(vex_token.transfer, 'vex.token contract');
        assert(vex_token.issue, 'vex.token contract');
        done();
      });
    });
  }
  checkContract('vexcore');
  checkContract('vex.token');
